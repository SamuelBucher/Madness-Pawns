...
namespace Madness_Pawns
{
    ...
    public static class HarmonyPatches
    {
        ...
        {
            [HarmonyPostfix]
            public static Color GetSkinColorPostfix(Color result)
            {
            ...
            public static void ResolveAllGraphicsPostfix(ref PawnGraphicSet __instance)
            {
                ...
                if (__instance.pawn.RaceProps.Humanlike)
                {
                    ...
                    if (__instance.pawn.gender == Gender.Female && LoadedModManager.GetMod<MadnessPawns>().GetSettings<Settings>().enableFemaleBodyType)
                        bodyTypePath = BodyTypeDefOf.Female.bodyNakedGraphicPath;
                    else
                        bodyTypePath = BodyTypeDefOf.Male.bodyNakedGraphicPath;
                    ...
                    __instance.nakedGraphic = GraphicDatabase.Get<Graphic_Multi>(bodyTypePath, ShaderUtility.GetSkinShader(__instance.pawn.story.SkinColorOverriden), Vector2.one, __instance.pawn.story.SkinColor);
                    __instance.rottingGraphic = GraphicDatabase.Get<Graphic_Multi>(bodyTypePath, ShaderUtility.GetSkinShader(__instance.pawn.story.SkinColorOverriden), Vector2.one, color);
                    __instance.dessicatedGraphic = GraphicDatabase.Get<Graphic_Multi>(bodyTypePath, ShaderDatabase.Cutout);
                    ...
                    if ((__instance.pawn.style != null && ModsConfig.IdeologyActive) && (__instance.pawn.style.BodyTattoo != null && __instance.pawn.style.BodyTattoo != TattooDefOf.NoTattoo_Body))
                    {
                        ...
                        skinColor.a *= 0.8f;
                        __instance.bodyTattooGraphic = GraphicDatabase.Get<Graphic_Multi>(__instance.pawn.style.BodyTattoo.texPath, ShaderDatabase.CutoutSkinOverlay, Vector2.one, skinColor, Color.white, null, bodyTypePath);
                    }
                    else
        ...
        class ResolveApparelGraphics
        {
            ...
            public static bool ResolveApparelGraphicsPrefix(ref PawnGraphicSet __instance)
            {
                ...
                BodyTypeDef bodyTypePath;

                if (__instance.pawn.gender == Gender.Female && LoadedModManager.GetMod<MadnessPawns>().GetSettings<Settings>().enableFemaleBodyType)
                    bodyTypePath = BodyTypeDefOf.Female;
                else
                    bodyTypePath = BodyTypeDefOf.Male;
                ...
                using (List<Apparel>.Enumerator enumerator = __instance.pawn.apparel.WornApparel.GetEnumerator())
                {
                    while (enumerator.MoveNext())
                    {
                        ...
                        if (ApparelGraphicRecordGetter.TryGetGraphicApparel(enumerator.Current, bodyTypePath, out item))
                        {
